...
Gemma 4 の 26.2万語彙（262,144語）巨大語彙テーブルは、日本語ルーティングにおいて決定的な優位性をもたらします。

- **トークン圧縮比の実測**:
  - クエリ例: `「暗号資産の出金制限と本人確認（KYC）のステータスを確認したい」`
  - Qwen2.5 (151k vocab): **22 トークン**
  - Gemma 4 (262k vocab): **14 トークン**（約36%のトークン長削減）
- **技術的恩恵**:
  Transformer の Prefill 計算量 $O(N^2)$ に対し、入力トークン長 $N$ が短縮されることで、語彙テーブルの大きさによるパラメータ増を相殺し、実機 Prefill 遅延を抑える効果が確認されました。

### 2.4 低VRAM環境向け量子化カスケード構成（Quantized Cascade Deployment）
実機 100問ベンチマークにより、量子化モデルを組み合わせた低VRAMカスケードの有効性が実証されました。

- **構成**:
  - **Tier-1**: `Qwen2.5-1.5B-Instruct-AWQ`（AWQ Marlin 4-bit, p50: **31.41 ms**, ピーク VRAM: **2.96 GB**, 正解率: **84.0%**）
  - **Tier-2**: `google/gemma-4-E2B-it` (bitsandbytes 4-bit, p50: **171.89 ms**, ピーク VRAM: **6.49 GB**, 正解率: **91.0%**)
- **数理モデルと期待遅延**:
  $$E[T] = 0.80 \times 31.41\,\text{ms} + 0.20 \times (31.41 + 171.89)\,\text{ms} = 25.13\,\text{ms} + 40.66\,\text{ms} = \mathbf{65.79\,\text{ms}}$$
- **運用上の意義**:
  両モデルを同時に単一 GPU 上にロードしても合計 VRAM は **約 9.45 GB** に収まり、RTX 3060 (12GB) や RTX 4070 などのコンシューマ向け GPU 1枚で、平均 65ms・実効精度 91% 超のカスケードルーティングが完全に自立稼働します。
